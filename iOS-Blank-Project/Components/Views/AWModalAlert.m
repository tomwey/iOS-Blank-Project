...
{
    [self showWithTitle:title message:message cancelButton:AWLocalizedString(@"No", nil)
           otherButtons:@[AWLocalizedString(@"Yes", nil)] result:^(NSUInteger buttonIndex) {
...
{
    [self showWithTitle:title message:message cancelButton:AWLocalizedString(@"Cancel", nil)
           otherButtons:@[AWLocalizedString(@"OK", nil)] result:^(NSUInteger buttonIndex) {
        result( (buttonIndex == 1) );
    }];
}
...
+ (void) say:(id)formatString,...
{
    ...
    [self showWithTitle:nil
                message:message
           cancelButton:AWLocalizedString(@"OK", nil)
           otherButtons:nil
                 result:nil];
